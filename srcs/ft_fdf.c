...
/*   Updated: 2016/12/09 01:26:11 by nboste           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		fdf_init(t_env *env, int width, int height)
{
	...
	if ((env->img = mlx_new_image(env->mlx, width, height)) == NULL)
		return (0);
	mlx_expose_hook(env->win, &fdf_expose_hook, (void *)env);
	mlx_key_hook(env->win, &fdf_key_hook, (void *)env);
	mlx_mouse_hook(env->win, &fdf_mouse_hook, (void *)env);
//	mlx_loop_hook(env->win, &fdf_loop_hook, (void *)env);
	return (1);
}
